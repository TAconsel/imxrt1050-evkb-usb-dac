...
uint8_t g_UsbDeviceConfigurationDescriptor[] = {
    ...
    */
    ...
    /* iTerminal = 0 on purpose. Pointing this at the product string made
     * snd-usb-audio name the mixer element "USB AUDIO DEMO Playback Volume",
     * which PulseAudio/PipeWire's ACP mixer paths do not recognise, so the
     * desktop slider fell back to software volume and never reached the codec.
     * With 0 the kernel derives the name from the terminal type (0x0101, USB
     * Streaming) and calls it "PCM Playback Volume", which ACP does map. */
    ...
    USB_AUDIO_FORMAT_TYPE_I, /* The format type AudioStreaming interfae using is FORMAT_TYPE_I (0x01)   */
    AUDIO_FORMAT_SIZE,       /* The number of bytes occupied by one audio subslot. Can be 1, 2, 3 or 4.   */
    AUDIO_FORMAT_BITS,       /* The number of effectively used bits from the available bits in an audio subslot   */
