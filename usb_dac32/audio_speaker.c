...
usb_audio_speaker_struct_t g_UsbDeviceAudioSpeaker = {
    ...
    .curMute20          = 0U,
    .curClockValid      = 1U,
    .curVolume20        = {0x00U, 0x00U}, /* 0 dB */
    .curSampleFrequency = AUDIO_SAMPLING_RATE,
    .freqControlRange   = {1U, AUDIO_SAMPLING_RATE, AUDIO_SAMPLING_RATE, 0U},
    /* WM8960 analog headphone range: -73 dB .. +6 dB in 1 dB steps, in UAC
     * 1/256 dB units. Reporting the real range (instead of the stock
     * +-127 dB / 1/256 dB) makes the host slider map 1:1 onto codec steps. */
    .volumeControlRange = {1U, 0xB700U, 0x0600U, 0x0100U},
#endif
...
#else
        BOARD_Codec_SetVolume((int16_t)((uint16_t)g_UsbDeviceAudioSpeaker.curVolume[1] << 8U |
                                        (uint16_t)g_UsbDeviceAudioSpeaker.curVolume[0]));
#endif
        g_UsbDeviceAudioSpeaker.codecTask &= ~VOLUME_CHANGE_TASK;
    }
}
